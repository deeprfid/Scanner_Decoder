...
//EPC CRC 计算
//-------------------------------------------------------------------------------------
uint16 crc_CalEPCcrc16(unsigned char *buf,unsigned short bitslength) //返回值为16位CRC
{
    uint16 shift,data,val;
    int i;
//  shift = CRC_SEED;	 //0XFFFF
    shift =0xffff;
...
uint8 crc_CalEPCcrc5(unsigned char *buf,unsigned short bitslength)	//返回值为5位CRC
{
    char shift=0;
    char data=0,val=0;
    int i=0;
...
{
    //uint16 calcCrc = CRC_SEED;
    uint16 calcCrc = 0xffff;
    uint8  i;

    for (i = 1; i < msglen; ++i)
        //CRC_calcCrc8(&calcCrc, POLY16, msgbuf[i]);
        crc_calcCrc8(&calcCrc,0x1021, msgbuf[i]);
