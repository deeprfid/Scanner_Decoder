                                ...
                                uint32_t u32Baudrate)
{
    ...
    if (IS_VALID_USART(USARTx))
    {
        if(1ul == USARTx->CR3_f.SCEN)
        {
            enRet = SetScBaudrate(USARTx, u32Baudrate);//智能卡模式
        }
        else if(1ul == USARTx->CR1_f.MS)
        {
            enRet = SetClkSyncBaudrate(USARTx, u32Baudrate);//时钟同步模式
        }
        else
        {
            enRet = SetUartBaudrate(USARTx, u32Baudrate);//UART模式
        }
    }
