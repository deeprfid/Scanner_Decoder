...
void timer_Delay_ms(unsigned int ms) //ms 延时：按 1ms 分块（TMRA3 16bit 上限 65535；原 3000*ONEUSC 在 ONEUSC>=22 溢出死循环）
{
    uint32 tm;
...
void timer4_ini(void)//512ms中断一次，用于ms 长时间计时
{
    ...
    stcTimeraInit.enCntDir = TimeraCountDirUp;
    stcTimeraInit.enSyncStartupEn = Disable;  //同步启动关闭
    stcTimeraInit.u16PeriodVal = (50000-1);  //512ms 溢出中断 @PCLK1 100MHz/1024
