...
int tagCmp(int cmpindex, TAGINFO *tag)
{
	...
	if (tb_IsUniByEmdData == 1)
	{
		if (tb_pTags[start+7+tag->Epclen] != tag->EmbededDatalen)
			return 1;
		if (memcmp(tb_pTags+start+8+tag->Epclen, tag->EmbededData, tag->EmbededDatalen) != 0)
			return 1;
	}
	return 0;
}
...
void initTbBuffer(int datalen)
{
	...
	tb_pTags = malloc(MaxTagBytesBufferLen);
	tb_ItemLen = 8+datalen;
	tb_MaxTagCount = MaxTagBytesBufferLen / tb_ItemLen;
	tb_pTagIndexHeaders = (unsigned short *)malloc(sizeof(unsigned short)*tb_MaxTagCount);
	printf("tb_MaxTagCount:%d,tb_pTags:%p, tb_pTagIndexHeaders:%p\n", 
