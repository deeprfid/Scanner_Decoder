/**
 * @file ota_layout.h
 * @brief F460 OTA 布局常量（QSPI 16MB + 片内 512KB），�? F4A0 同构命名
 *
 * QSPI 16MB 分区（已定�?�）�?
 *   暂存�? 0x000000-0x07FFFF (512KB)  新固�?
 *   备份�? 0x080000-0x0FFFFF (512KB)  升级前旧固件备份
 *   FlashDB 0x100000-0x8FFFFF (8MB)  参数/配置
 *   网页/FS 0x900000-0xEFFFFF (6MB)  index.html / 标�?? CSV
 *   备用    0xF00000-0xFFFFFF (1MB)  OTA 状�?�扇区等
 *
 * 片内 512KB：App 0x16000 起（boot 跳转地址）；标志区待定（App 区末�?扇区�?
 */
#ifndef OTA_LAYOUT_H
#define OTA_LAYOUT_H
...
/* 升级状�?�扇区（备用区头�?�?4KB 扇区对齐�? */
#define OTA_STATE_SECTOR_BASE  0x00100000UL   /* ״̬��: ��������, W25Q64 8MB �� */
#define OTA_STATE_SECTOR_SIZE  0x00001000UL   /* 4KB */   /* 4KB */

/* ---- 片内 ---- */
#define OTA_APP_BASE           0x00010000UL   /* App 链接基址（boot run_app 跳转地址�? */
#define OTA_APP_MAX_SIZE       0x00070000UL   /* App 区上限（512KB-起�?�偏移，预留标志扇区�? */

/* F460 固件版本（与打包 ota_pack.py --version �?致；F460 首个 OTA 版本�? */
