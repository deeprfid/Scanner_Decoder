...
2ï¼šfirmware:  å…³é—­init_usb(1);init_usb(gPRdrStaSet->app_init.usb_type);
...
void user_main(void)
{
	...
	wait_fin_init();
	//ota_agent_boot();      /* OTA ÔÝÊ±½ûÓÃ */
	//ota_server_start();   /* OTA ÔÝÊ±½ûÓÃ */
	brdcst_conf_init(COMMON_INTERFACE_SOCKET3);   /* ¹ã²¥¹Ì¶¨ socket3£¨¶ÔÆë F4A0£¬±ÜÃâ¶¯Ì¬ºÅÓë OTA socket2 ³åÍ»£© */
