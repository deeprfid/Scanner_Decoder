...
#endif
/* 自检用最后 4KB 扇区（0x7FF000，避开暂存区与未来分区） */
...
/* Device ID（0x90 双字节 (MFR<<8)|DevID，与 App w25qxx.h 表一致） */
#define W25Q64_DEV_ID           0xEF16u   /* W25Q64  8MB (量产型) */
 ...
 * @note  执行一次擦写测试（写完回读比对后擦除还原），结果全打印
 */
void boot_qspi_diag(void)
{
    ...
    /* 1b) Device ID (0x90，双字节) —— 用 App w25qxx.h 同款宏判断 */
    {
        uint16_t u16DevId;
    ...
    /* 4) 分步擦写测试 @0x7FF000（fresh 片=FF，可独立验证 编程/擦除） */
    for (i = 0UL; i < sizeof(au8Pat); i++) {
        au8Pat[i] = (uint8_t)(0xA0U + i);
    }

    /* A) WREN 锁存检查：0x06 后 SR1.WEL(bit1) 应为 1 */
    {
        uint8_t u8Wren = W25Q_WRITE_ENABLE;
        qspi_cmd(&u8Wren, 1U);
